...
REGLAS: Modo socrático — guiá con preguntas y pistas incrementales; NO des la solución completa del ejercicio salvo pedido explícito y en ese caso avisá que conviene intentarlo antes. Respuestas cortas (2-5 oraciones). Solo temas de la lección o prerequisitos.`;
      ...
      // Llama a la serverless function de Vercel (la API key vive en el server).
      ...
      const reply = data.text || "No pude generar respuesta.";
...
export default function StudyApp({ session }) {
  ...
  const [current, setCurrent] = useState("L01");
  // Progreso independiente por materia (Set de códigos aprobados)
  const [completed, setCompleted] = useState({});
  // Mapa `${materia}/${code}` -> lesson_id (uuid) para las FK de progreso.
  const [keyToId, setKeyToId] = useState({});
  const codeRef = useRef(getLesson("PRG1", "L01").exercise.starter);
  const sessionIdRef = useRef(null);
  const startedAtRef = useRef(Date.now());
  const userId = session?.user?.id;

  // Al montar: carga IDs de lecciones y progreso completado, y abre sesion.
  useEffect(() => {
    let cancel = false;
    (async () => {
      try {
        const { keyToId, idToKey } = await fetchLessonMaps();
        if (cancel) return;
        setKeyToId(keyToId);
        const byMateria = await loadCompleted(idToKey);
        if (cancel) return;
        setCompleted(byMateria);
      } catch (e) {
        console.error("No se pudo cargar el progreso:", e.message);
      }
    })();
    startStudySession(userId).then((id) => { sessionIdRef.current = id; });
    const cerrar = () => {
      const mins = Math.round((Date.now() - startedAtRef.current) / 60000);
      endStudySession(sessionIdRef.current, mins);
    };
    window.addEventListener("beforeunload", cerrar);
    return () => { cancel = true; window.removeEventListener("beforeunload", cerrar); cerrar(); };
  }, [userId]);
  ...
  function handlePass(materia, code) {
    ...
    const lessonId = keyToId[`${materia}/${code}`];
    if (userId && lessonId) {
      markLessonCompleted({ userId, lessonId, lessonCode: code }).catch((e) =>
        console.error("No se pudo guardar el progreso:", e.message)
      );
    }
  }
  ...
  const header = (
    <div style={{
      ...
      <button onClick={logout} style={{
        marginLeft: 12, padding: "4px 10px", borderRadius: 5, border: `1px solid ${T.line}`,
        background: "transparent", color: "#8b98a5", fontFamily: T.mono, fontSize: 10.5,
